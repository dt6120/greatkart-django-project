...
def register(request):
    if request.method == 'POST':
      ...
      if form.is_valid():
        ...
        #   messages.success(request, 'thank you for Registration. Verify it.')
...
def login(request):
    if request.method == 'POST':
       ...
       if user is not None:
           try:
               ...
               if is_cart_item_exists:
                  ...
                  print(cart_item)
                    ...
                    # getting the product varaiation from cart id
                  product_variation = []
           ...
           url = request.META.get('HTTP_REFERAL')
           try:
...
def activate(request, uidb64, token):
    try:
        ...
        user = Account._default_manage.get(pk=uid)
...
def forgotPassword(request):
    if request.method == 'POST':
        ...
        if Account.objects.filter(email=email).exists():
            ...
            messages.error(request, 'Account does not exixts.')
